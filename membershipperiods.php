		...
		if (!($result->fetch())) {
			...
			if($resultc->fetch()) {
				if(!empty($resultc->new_date))
					...
					$query = "INSERT INTO civicrm_membership_period (start_date, end_date, membership_id, contribution_id, renewed_date) VALUES ('$objectRef->start_date', '$objectRef->end_date', $objectRef->id, 0, NOW())";	
